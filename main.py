...
img = cv2.imread('./CASTELO_01.jpeg')
...
# img_gray = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
# a = img_gray.max()
# _, thresh = cv2.threshold(img_gray, a/2*1.35, a,cv2.THRESH_BINARY_INV)
#
# tamanhoKernel = 5
# kernel = np.ones((tamanhoKernel,tamanhoKernel), np.uint8)
# thresh_open = cv2.morphologyEx(thresh, cv2.MORPH_OPEN, kernel)
...
#
# # Detecção borda com Canny (sem blurry)
# edges_gray = cv2.Canny(image=img_gray, threshold1=a/0.7, threshold2=a/0.7)
# # Detecção borda com Canny (com blurry)
# edges_blur = cv2.Canny(image=img_blur, threshold1=a/0.7, threshold2=a/0.7)
#
# # contorno
# contours, hierarchy = cv2.findContours(
#                                    image = thresh,
